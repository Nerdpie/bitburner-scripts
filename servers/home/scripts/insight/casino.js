// Copied from https://github.com/alainbryden/bitburner-scripts/blob/main/casino.js
// noinspection ES6ConvertLetToConst,FunctionWithInconsistentReturnsJS,ExceptionCaughtLocallyJS,EqualityComparisonWithCoercionJS,EmptyCatchBlockJS,NonBlockStatementBodyJS,SpellCheckingInspection,JSUnusedAssignment,IncrementDecrementResultUsedJS,PointlessBooleanExpressionJS,UnreachableCodeJS,NestedConditionalExpressionJS,JSCommentMatchesSignature,DynamicallyGeneratedCodeJS,MagicNumberJS
...
export async function main(ns) {
   ...
   * @param {boolean} silent (default false) Set to true to suppress the warning popup if throwError is false and something has focus.
   ...
   *                  If false, it will log a warning, try to stop any focus work (up to `retries` times), then return true.
   * @param {boolean} silent (default false) Set to true if we fully expect not to be at the casino yet, so we don't want to log a warning if that is the case.
