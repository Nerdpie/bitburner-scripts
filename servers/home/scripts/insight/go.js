...
export async function main(ns) {
  ...
  function rotate90Degrees(pattern) {
    return pattern.map((_val, index) => pattern.map(row => row[index]).reverse().join(""))
  }
  /** @param {NS} ns
   * @param {string[]} pattern
   * @returns {string[]} */
